...
2019. The analysis is based around the `rtweet` package, and the code is
...
twitter_graph %>% 
  ...
  theme(plot.title = element_text(size = rel(1.5), face = "bold", margin = margin(3, 1, 10, 1)),
      plot.subtitle = element_text(size = rel(1.2)),
      plot.caption = element_text(colour = "#444444")) +
  labs(title = "Connections between Twitter users during SER 2019",
    caption = "\n\nSource: Data gathered via Twitter's standard `search/tweets` API using rtweet\n Code provided by Mike Kearney (@kearneymw)",
    x = NULL, y = NULL) 
...
For after the conference, here’s an animated version (not currently
evaluated):
...
animated_plot <- twitter_graph %>% 
  ...
  theme(plot.title = element_text(size = rel(1.5), face = "bold", margin = margin(3, 1, 10, 1)),
      plot.subtitle = element_text(size = rel(1.2)),
      plot.caption = element_text(colour = "#444444")) +
  transition_events(start = time, enter_length = hms::hms(hours = 5), exit_length = hms::hms(hours = 5)) +
  enter_fade() + 
  exit_fade()
  
animate(animated_plot, 300, 10)
```
